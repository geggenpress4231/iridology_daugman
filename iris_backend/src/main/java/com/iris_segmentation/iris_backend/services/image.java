...
public class image {
    ...
    public image(File img) throws IOException{
    ...
    public void fromBufferedImage(BufferedImage img) {
        // Set width and height from BufferedImage
        this.w = img.getWidth();
        this.h = img.getHeight();
        this.colors = new int[3][h][w];

        // Extract RGB values and store them in colors array
        for (int i = 0; i < h; i++) {
            for (int j = 0; j < w; j++) {
                int rgb = img.getRGB(j, i);
                Color color = new Color(rgb, true);
                this.colors[0][i][j] = color.getRed();
                this.colors[1][i][j] = color.getGreen();
                this.colors[2][i][j] = color.getBlue();
            }
        }
    }
